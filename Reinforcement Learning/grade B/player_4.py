...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        linear_schedule = ScheduleLinear(
    ...
    possible_moves = [Q[state][idx] if idx in all_actions else np.nan for idx in range(len(Q[state]))]
    any_action = random.choice(all_actions)
    action = epsilon*any_action + (1-epsilon)*np.nanargmax(possible_moves)
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max:
            ...
            R_total = 0
            steps = 0
            while not end_episode and self.threshold <= diff:
                # selection of action
                ...
                alpha, gamma = self.settings.alpha, self.settings.gamma
                error = alpha*(R + gamma*np.nanmax(Q[s_next]) - Q[s_current][action])
                Q[s_current][action] = Q[s_current][action] + error
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
    ...
    def random_agent(self):
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                action = possible_actions[random.randint(0, na)]
                # ADD YOUR CODE SNIPPET BETWEEN EX. 1.2
...
class ScheduleLinear(object):
    def __init__(self, schedule_timesteps=10000, final_p=0.2, initial_p=1.0):
    ...
    def value(self, t):
        ...
        epsilon_t = self.initial_p + (self.final_p-self.initial_p)*t/self.schedule_timesteps
        return 1 if np.random.uniform(low=0.0, high=1.0) < epsilon_t else 0 # Return epsilon=1 (extrapolation) or epsilon=0 (explotation)
        # ADD YOUR CODE SNIPPET BETWEEN EX 3.2