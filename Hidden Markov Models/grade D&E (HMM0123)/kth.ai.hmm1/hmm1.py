...
# A_input = [4.0, 4.0, 0.0, 0.8, 0.1, 0.1, 0.1, 0.0, 0.8, 0.1, 0.1, 0.1, 0.0, 0.8, 0.8, 0.1, 0.1, 0.0]
# B_input = [4.0, 4.0, 0.9, 0.1, 0.0, 0.0, 0.0, 0.9, 0.1, 0.0, 0.0, 0.0, 0.9, 0.1, 0.1, 0.0, 0.0, 0.9]
# pi_input = [1.0, 4.0, 1.0, 0.0, 0.0, 0.0]
# obs_input = [8.0, 0.0, 1.0, 2.0, 3.0, 0.0, 1.0, 2.0, 3.0]
...
for idx in range(len(seq_obs)):
    obs = seq_obs[idx]
    alphaA = multiplication(alpha, A, 1, ma, na) if idx>0 else pi # row vector
    B_obs = transpose(B, mb, nb)[obs] # row vector
    alpha = dot_vect(alphaA, B_obs)
